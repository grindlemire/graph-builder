...
A dependency graph execution engine in Go designed for **multi-team ownership**. Nodes self-register via `init()` functions, enabling decentralized development where teams can add nodes without touching shared orchestration code.
...
```go
import (
    _ "github.com/grindlemire/graph-builder/pkg/nodes/node1"
    _ "github.com/grindlemire/graph-builder/pkg/nodes/node2a"
    // ... one line per node
)
```
...
2. **Add one import line to `nodes.go`:**
   ...
   ```go
   _ "github.com/grindlemire/graph-builder/pkg/nodes/mynode"
   ```
...
```go
import "github.com/grindlemire/graph-builder/pkg/nodes/node2a"
